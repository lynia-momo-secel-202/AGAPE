define(function (require) {
    ...
    var FilieresModel = require('../../model/Filieres');
    ...
    var ThemeRiverFilieres = FilieresModel.extend({
        ...
        init: function (option) {
            ThemeRiverFilieres.superApply(this, 'init', arguments);
        ...
        getLayerFilieres: function () {
            ...
            var layerFilieres = zrUtil.map(dataByName, function (d) {
                return {
                    name: d.key,
                    indices: d.values
                };
            ...
            for(var j = 0; j < layerFilieres.length; ++j) {
                layerFilieres[j].indices.sort(comparer);
            }
        ...
        getAxisTooltipDataIndex: function (dim, value, baseAxis) {
            ...
            var layerFilieres = this.getLayerFilieres();
            var indices = [];
            var layerNum = layerFilieres.length;

            for (var i = 0; i < layerNum; ++i) {
                var minDist = Number.MAX_VALUE;
                var nearestIdx = -1;
                var pointNum = layerFilieres[i].indices.length;
                for (var j = 0; j < pointNum; ++j) {
                    var dist = Math.abs(data.get(dim[0], layerFilieres[i].indices[j]) - value);
                    if (dist <= minDist) {
                        minDist = dist;
                        nearestIdx = layerFilieres[i].indices[j];
                    }
                }
