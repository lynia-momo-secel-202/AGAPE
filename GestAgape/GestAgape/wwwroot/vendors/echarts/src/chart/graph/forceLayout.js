define(function (require) {
    ...
    return function (ecModel, api) {
        ecModel.eachFilieresByType('graph', function (graphFilieres) {
            var coordSys = graphFilieres.coordinateSystem;
            if (coordSys && coordSys.type !== 'view') {
                return;
            }
            if (graphFilieres.get('layout') === 'force') {
                var preservedPoints = graphFilieres.preservedPoints || {};
                var graph = graphFilieres.getGraph();
                var nodeData = graph.data;
                var edgeData = graph.edgeData;
                var forceModel = graphFilieres.getModel('force');
                var initLayout = forceModel.get('initLayout');
                if (graphFilieres.preservedPoints) {
                    ...
                    simpleLayoutHelper(graphFilieres);
                }
                else if (initLayout === 'circular') {
                    circularLayoutHelper(graphFilieres);
                }
                ...
                graphFilieres.forceLayout = forceInstance;
                graphFilieres.preservedPoints = preservedPoints;
                ...
                graphFilieres.forceLayout = null;
            }
        });
    };
});