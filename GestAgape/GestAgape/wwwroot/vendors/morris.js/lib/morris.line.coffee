class Morris.Line extends Morris.Grid
  ...
  drawXAxis: ->
    ...
    if @options.parseTime
      if @data.length == 1 and @options.xLabels == 'auto'
        ...
        labels = Morris.labelFilieres(@xmin, @xmax, @width, @options.xLabels, @options.xLabelFormat)
    else
    ...
    if @prevHilight isnt null and @prevHilight isnt index
      for i in [0..@seriesPoints.length-1]
        if @seriesPoints[i][@prevHilight]
          @seriesPoints[i][@prevHilight].animate @pointShrinkFilieres(i)
    if index isnt null and @prevHilight isnt index
      for i in [0..@seriesPoints.length-1]
        if @seriesPoints[i][index]
          @seriesPoints[i][index].animate @pointGrowFilieres(i)
    @prevHilight = index
  ...
  drawLinePath: (path, lineColor, lineIndex) ->
    @raphael.path(path)
      ...
      .attr('stroke-width', @lineWidthForFilieres(lineIndex))

  drawLinePoint: (xPos, yPos, pointColor, lineIndex) ->
    @raphael.circle(xPos, yPos, @pointSizeForFilieres(lineIndex))
      .attr('fill', pointColor)
      .attr('stroke-width', @pointStrokeWidthForFilieres(lineIndex))
      .attr('stroke', @pointStrokeColorForFilieres(lineIndex))

  # @private
  pointStrokeWidthForFilieres: (index) ->
    @options.pointStrokeWidths[index % @options.pointStrokeWidths.length]

  # @private
  pointStrokeColorForFilieres: (index) ->
    @options.pointStrokeColors[index % @options.pointStrokeColors.length]

  # @private
  lineWidthForFilieres: (index) ->
  ...
  # @private
  pointGrowFilieres: (index) ->
    Raphael.animation r: @pointSizeForFilieres(index) + 3, 25, 'linear'

  # @private
  pointShrinkFilieres: (index) ->
    Raphael.animation r: @pointSizeForFilieres(index), 25, 'linear'

# generate a series of label, timestamp pairs for x-axis labels
#
# @private
Morris.labelFilieres = (dmin, dmax, pxwidth, specName, xLabelFormat) ->
