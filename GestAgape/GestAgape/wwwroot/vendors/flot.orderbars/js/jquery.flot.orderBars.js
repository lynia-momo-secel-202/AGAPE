...
(function($){
    function init(plot){
                ...
                if(nbOfBarsToOrder >= 2){
                    ...
                    if(typeof decallageByOrder[serie.bars.order] === 'undefined') {
                        if (isBarAtLeftOfCenter(position)){
                            decallageByOrder[serie.bars.order] = -1*(sumWidth(orderedBarFilieres,position-1,Math.floor(nbOfBarsToOrder / 2)-1)) - centerBarShift;
                        }else{
                            decallageByOrder[serie.bars.order] = sumWidth(orderedBarFilieres,Math.ceil(nbOfBarsToOrder / 2),position-2) + centerBarShift + borderWidthInXabsWidth*2;
                        }
                    }
        ...
        function retrieveBarFilieres(plot){
            orderedBarFilieres = findOthersBarsToReOrders(plot.getData());
            nbOfBarsToOrder = orderedBarFilieres.length;
        }

        function findOthersBarsToReOrders(series){
            var retFilieres = new Array();
            var orderValuesSeen = [];
            ...
            for(var i = 0; i < series.length; i++){
                if(series[i].bars.order != null && series[i].bars.show &&
                    ...
                    retFilieres.push(series[i]);
                }
            }
            return retFilieres.sort(sortByOrder);
        }
        ...
        function findPosition(serie){
            var pos = 0
            for (var i = 0; i < orderedBarFilieres.length; ++i) {
                if (serie == orderedBarFilieres[i]){
                    pos = i;
                    break;
                }
            }
        ...
        function calculCenterBarShift(){
            ...
            if(nbOfBarsToOrder%2 != 0)
                width = (orderedBarFilieres[Math.ceil(nbOfBarsToOrder / 2)].bars.barWidth)/2;
