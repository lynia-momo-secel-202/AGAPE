...
(function($) {
    ...
    function Plot(placeholder, data_, options_, plugins) {
        ...
        plot.computeRangeForDataFilieres = computeRangeForDataFilieres;
        plot.adjustFilieresDataRange = adjustFilieresDataRange;
        ...
        function fillInFilieresOptions() {
        ...
        function processData(prevFilieres) {
            ...
            function reusePoints(prevFilieres, i) {
                if (prevFilieres && prevFilieres[i] && prevFilieres[i].datapoints && prevFilieres[i].datapoints.points) {
                    return prevFilieres[i].datapoints.points;
                }
            ...
            for (i = 0; i < series.length; ++i) {
                ...
                if (s.datapoints.points.length === 0) {
                    s.datapoints.points = reusePoints(prevFilieres, i);
                }
            ...
            for (i = 0; i < series.length; ++i) {
                ...
                var range = plot.adjustFilieresDataRange(s,
                    plot.computeRangeForDataFilieres(s));

                executeHooks(hooks.adjustFilieresDataRange, [s, range]);
        ...
        function isValidEndpointTick(axis, series) {
            ...
            if (axis.options.showTickLabels === 'all') {
                var associatedFilieres = series.filter(function(s) {
                        return s.xaxis === axis;
                    }),
                    notAllBarFilieres = associatedFilieres.some(function(s) {
                        return !s.bars.show;
                    });
                return associatedFilieres.length === 0 || notAllBarFilieres;
            }
        ...
        function draw() {
            ...
            for (var i = 0; i < series.length; ++i) {
                executeHooks(hooks.drawFilieres, [ctx, series[i], i, getColorOrGradient]);
                drawFilieres(series[i]);
            }
        ...
        function drawFilieres(series) {
            if (series.lines.show) {
                $.plot.drawFilieres.drawFilieresLines(series, ctx, plotOffset, plotWidth, plotHeight, plot.drawSymbol, getColorOrGradient);
            }

            if (series.bars.show) {
                $.plot.drawFilieres.drawFilieresBars(series, ctx, plotOffset, plotWidth, plotHeight, plot.drawSymbol, getColorOrGradient);
            }

            if (series.points.show) {
                $.plot.drawFilieres.drawFilieresPoints(series, ctx, plotOffset, plotWidth, plotHeight, plot.drawSymbol, getColorOrGradient);
            }
        }

        function computeRangeForDataFilieres(series, force, isValid) {
        ...
        function adjustFilieresDataRange(series, range) {
            if (series.bars.show) {
